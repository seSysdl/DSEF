...
class UnifiedClAdv(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform):
        super(UnifiedClAdv, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch=None):
        ...
        if not self.buffer.is_empty():
            ...
            aOutputs = self.net(aSamples)
            loss3 = self.loss(aOutputs, buf_labels)
            #loss3.backward()
            loss += loss3
            # End loss to the adversarial example
    ...
    def unlearn(self, inputs, labels):
        ...
        for (model_name, model_param), (temp_name, temp_param) in zip(self.net.named_parameters(),
            ...
            with torch.no_grad():
                model_param.add_(self.delta * torch.mul(1.0 / (identity + 0.001 * self.fish[model_name]),
                                                        norm_update + 0.001 * torch.randn_like(norm_update)))
