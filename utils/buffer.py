...
def loss_sampling(num_seen_examples: int, buffer_size: int) -> int:
    """
    Loss-based sampling algorithm by using gradient norm and entropy.
    ...
    if num_seen_examples < buffer_size:
        ...
        rand = np.random.randint(0, buffer_size)
        return rand
...
class Buffer:
    ...
    def add_data_byLoss(self, examples, labels=None, task_labels=None, logits=None, attention_maps=None, index_list=[]):
        """
        Adds the data to the memory buffer according to the loss function value.
        ...
        Args:
            ...
            logits: tensor containing the outputs of the network
            task_labels: tensor containing the task labels
            index_list: list of indexes to replace by loss value from large to small
        ...
        for i in index_list:
            index = loss_sampling(self.num_seen_examples, self.buffer_size)
            self.num_seen_examples += 1
            if index >= 0:
                self.examples[index] = examples[i].to(self.device)
                if labels is not None:
