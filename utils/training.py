    ...
    for k, test_loader in enumerate(dataset.test_loaders):
        ...
        while True:
            ...
            if 'class-il' not in model.COMPATIBILITY and 'general-continual' not in model.COMPATIBILITY:
                ...
                if model.NAME == 'dynamicdefense':
                    outputs = model.myPrediction(inputs,k)
                elif model.NAME == 'dynamicdefense2' :
                    outputs = model.myPrediction(inputs, k)
                elif model.NAME == 'dynamicmodel':
                    outputs = model.myPrediction(inputs, k)
                elif model.NAME == 'dynamicmodeladv':
                    outputs = model.myPrediction(inputs, k)
                elif model.NAME == 'featuredisentangle':
                    outputs = model.myPrediction(inputs, k)
                elif model.NAME == 'robustdualvit':
...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    if model.NAME == 'dynamicdefense' or model.NAME == 'dynamicdefense2' or model.NAME == 'dynamicmodel' or model.NAME == 'dynamicmodeladv' or model.NAME == 'featuredisentangle' or model.NAME == 'robustdualvit':
